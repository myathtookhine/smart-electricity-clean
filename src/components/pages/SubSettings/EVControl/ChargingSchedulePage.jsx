...
export function ChargingSchedulePage({ mode, modeData, onBack, onComplete, tariffName }) {
    ...
    <ThemeProvider theme={createCustomTheme()}>
      ...
      <div className="px-6 pt-8 pb-6">
        <div className="flex items-center space-x-3 mb-2">
          ...
          <div className={`w-12 h-12 bg-gradient-to-br ${colors.bgGradient} rounded-2xl flex items-center justify-center shadow-lg`}>
            <modeData.icon className="w-7 h-7 text-white" />
          </div>
          <div>
            <h1 className="text-xl text-foreground font-semibold">Charging Schedule</h1>
            <p className="text-sm text-muted-foreground">
              {modeData.name}
              {mode === 'tariff-intelligence' && tariffName && ` • ${tariffName}`}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="px-6 space-y-6 pb-24">
        ...
        <div className="bg-card/50 backdrop-blur-sm rounded-3xl p-6 shadow-xl border border-border/50">
          <h3 className="text-lg text-card-foreground font-semibold mb-4">Charging Period</h3>
          <p className="text-sm text-muted-foreground mb-6">Set the start and end times for charging</p>
          
          {/* Time Range Display */}
          <div className="text-center mb-8">
            <div className="text-2xl font-bold text-foreground">
              {formatTime(scheduleForm.startTime)} - {formatTime(scheduleForm.endTime)}
            </div>
            <p className="text-sm text-muted-foreground mt-1">Charging time period</p>
          </div>
          ...
          <LocalizationProvider dateAdapter={AdapterDayjs}>
            <div className="space-y-6">
              ...
              <div className="bg-muted/10 rounded-2xl p-6">
                <h4 className="text-md font-medium text-foreground mb-4 text-center">Start Time</h4>
                <div className="flex justify-center">
                  <TimePicker
                    value={scheduleForm.startTime}
                    onChange={(newValue) => setScheduleForm({
                      ...scheduleForm,
                      startTime: newValue
                    })}
                  />
              ...
              {/* End Time */}
              <div className="bg-muted/10 rounded-2xl p-6">
                <h4 className="text-md font-medium text-foreground mb-4 text-center">End Time</h4>
                <div className="flex justify-center">
                  <TimePicker
        ...
        {/* Frequency */}
        <div className="bg-card/50 backdrop-blur-sm rounded-3xl p-6 shadow-xl border border-border/50">
          <h3 className="text-lg text-card-foreground font-semibold mb-4">Frequency</h3>
            ...
            {frequencyOptions.map((option) => (
              <button
                ...
                onClick={() => setScheduleForm({
                  ...
                  specificDays: option.value === 'specific-days' ? scheduleForm.specificDays : []
                })}
          ...
          {scheduleForm.frequency === 'specific-days' && (
            <div className="space-y-3">
              <label className="block text-sm text-muted-foreground">Select Days</label>
              <div className="grid grid-cols-7 gap-2">
                {daysOfWeek.map((day) => (
                  <button
                    key={day.value}
                    onClick={() => {
                      const isSelected = scheduleForm.specificDays.includes(day.value);
                      const newDays = isSelected
                        ? scheduleForm.specificDays.filter(d => d !== day.value)
                        : [...scheduleForm.specificDays, day.value];
                      setScheduleForm({ ...scheduleForm, specificDays: newDays });
                    }}

...
export function TariffSettingPage({ onBack, onComplete }) {
    ...
    <ThemeProvider theme={createCustomTheme()}>
      ...
      <div className="px-6 pt-8 pb-6">
        <div className="flex items-center space-x-3 mb-2">
          ...
          <div>
            <h1 className="text-xl text-foreground font-semibold">Tariff Settings</h1>
            <p className="text-sm text-muted-foreground">Configure your tariff rates</p>
          </div>
        </div>
      </div>
      ...
      <div className="px-6 space-y-6 pb-24">
        ...
        <div className="bg-card/50 backdrop-blur-sm rounded-3xl p-6 shadow-xl border border-border/50">
          <h3 className="text-lg text-card-foreground font-semibold mb-4">Low-Cost Period</h3>
          <p className="text-sm text-muted-foreground mb-6">Charges during low-cost tariff periods for optimal savings</p>
          
          {/* Time Range Display */}
          <div className="text-center mb-8">
            <div className="text-2xl font-bold text-foreground">
              {formatTime(tariffData.lowCostStartTime)} - {formatTime(tariffData.lowCostEndTime)}
            </div>
            <p className="text-sm text-muted-foreground mt-1">Low-cost charging period</p>
          </div>
              ...
              <div className="bg-muted/10 rounded-2xl p-6">
                ...
                <div className="flex justify-center">
                  <TimePicker
                    ...
                    onChange={(newValue) => setTariffData({
                      ...tariffData,
        ...
        <div className="bg-card/50 backdrop-blur-sm rounded-3xl p-6 shadow-xl border border-border/50">
          ...
          <div className="grid grid-cols-2 gap-3 mb-4">
            {frequencyOptions.map((option) => (
              <button
                key={option.value}
                onClick={() => setTariffData({
                  ...tariffData,
                  frequency: option.value,
                  specificDays: option.value === 'specific-days' ? tariffData.specificDays : []
                })}
              ...
              <div className="grid grid-cols-7 gap-2">
                {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map((day, index) => (
                  <button
                    key={day}
                    onClick={() => {
                      const newSpecificDays = tariffData.specificDays.includes(index)
                        ? tariffData.specificDays.filter(d => d !== index)
